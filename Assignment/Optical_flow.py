...
def disp_result(img, edge_img, motion_vec):
    ...
    for r in range(1, motion_vec.shape[0]-1):
        for c in range(1, motion_vec.shape[1]-1):
            if np.any(edge_img[r-1:r+2,c-1:c+2] >= -1): # np.any(edge_img[r-1:r+2,c-1:c+2] > 100): // edge_img[r,c] >= 0
                color = 'black' if np.linalg.norm(motion_vec[r,c]) < 0.5 else 'deeppink'
                ax.add_patch(
                    patches.Arrow(
...
def disp_result2(img, edge_img, motion_vec):
    plt.style.use('default')
    fig, ax = plt.subplots()
    for a in range(3):
        if a == 0:
        ...
        for r in range(1, motion_vec.shape[0]-1):
            for c in range(1, motion_vec.shape[1]-1):
                if np.any(edge_img[r-1:r+2,c-1:c+2] >= 100): # np.any(edge_img[r-1:r+2,c-1:c+2] > 100):
                    color = 'black' if np.linalg.norm(motion_vec[r,c,a]) < 0.3 else clo
                    ax.add_patch(
                        patches.Arrow(
                            c, motion_vec.shape[0]-(r+1),
                            motion_vec[r,c,a,1], -motion_vec[r,c,a,0],
                            width=0.2,
                            edgecolor=color,
                            facecolor='white'
                        ))
...
def main(mode):
    ...
    if mode == 0:
        ...
        ## display motion vector and image
        disp_result(A1_img_rgb, A1_edge_img, motion_vec_img)
        disp_result2(A1_img_rgb, A1_edge_img, motion_vec_set)
